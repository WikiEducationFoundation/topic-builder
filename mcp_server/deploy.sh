...
$SCP_CMD "$SCRIPT_DIR/server.py" "$SCRIPT_DIR/oauth.py" "$SCRIPT_DIR/topics_ui.py" "$SCRIPT_DIR/iv_packages.py" "$SCRIPT_DIR/csv_export.py" "$SCRIPT_DIR/wikipedia_api.py" "$SCRIPT_DIR/db.py" "$SCRIPT_DIR/server_instructions.md" "$SCRIPT_DIR/shape_axes.md" "$SCRIPT_DIR/strategy_moves.md" "$SCRIPT_DIR/failure_modes.md" "$SCRIPT_DIR/requirements.txt" "$DEPLOY_USER@$DEPLOY_HOST:$REMOTE_DIR/app/"
...
upstream topic_builder_backend {
    # Sticky session routing by CLIENT IP, not session-id. MCP keeps
    # per-session state in an in-memory dict per worker process
    # (StreamableHTTPSessionManager._server_instances), keyed by the
    # Mcp-Session-Id header. The session ID is generated by the
    # worker that handles the initial init request — so the worker
    # the client gets routed to FIRST must remain the worker for
    # all subsequent requests in that session.
    #
    # An earlier attempt used 'hash \$http_mcp_session_id consistent'
    # but that's broken: the initial init has no session ID (hashes
    # to a fixed worker), and the session ID then generated by that
    # worker may hash back to the OTHER worker for follow-up requests
    # → 404 'Session not found'. ip_hash sidesteps this entirely:
    # all requests from one client IP land on one worker, so init
    # and follow-ups always agree.
    #
    # Tradeoff: clients behind the same NAT all hash to the same
    # worker. For 5–10 operator sessions across a few locations
    # this is fine. If we ever need finer-grained sticky routing
    # we can revisit (sticky cookies via nginx-plus, or coordinate
    # session-id generation with the hash ring).
    ip_hash;
...
server {
    ...
    # Impact Visualizer handoff: server-to-server JSON endpoint serving
    # frozen packages by handle. Auth is handle unguessability — the
    # URL is a capability. Single SQLite read + JSON serialize; default
    # timeouts are fine.
    location /packages/ {
        proxy_pass http://topic_builder_backend;
        proxy_set_header Host \$host;
