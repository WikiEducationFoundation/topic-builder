...
run_topic_name_template: apollo-11-thin {ts}
---

# Benchmark run: Apollo 11
...
1. `start_topic(name="apollo-11-thin {ts}", wiki="en", fresh=False)`. The name above is pre-rendered with a fresh timestamp every time this brief is fetched — use it verbatim. A separate frozen baseline topic exists with a related name; do NOT overwrite it.
