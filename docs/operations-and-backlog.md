...
- **2026-04-22** — `wikidata_search_entity` QID/PID discovery helper (`[plan 5.1a]`). Wraps `wbsearchentities` — single API call, ~100ms. Addresses the 5.1 checkpoint's "picking the right QID is the hard part" failure mode. `entity_type="property"` mode finds P-IDs by name too.
- **2026-04-22** — Stage 5 pre-checkpoint: Wikidata substrate + QID backfill (`[plan 5.1, 5.6]`). `wikidata_entities_by_property(prop, value)` helper + raw `wikidata_query(sparql)` tool backed by shared SPARQL wrapper with 1h TTL cache. Articles table gains nullable `wikidata_qid` column; `resolve_qids(limit, time_budget_s)` tool does lazy pageprops backfill. Enriched CSV gets QID as 2nd column. Checkpoint finding: helper plumbing is solid; picking the right QID for a concept is the AI's hard part — flagged `wikidata_search_entity` as a QoL post-checkpoint add. Stage 5 paused here until Sage confirms post-checkpoint shape for 5.2/5.3/5.5.
- **2026-04-22** — Stage 4: two-axis topic model — inclusion × centrality (`[plan 4.1–4.7]`). Reshaped from "scoring at scale" decision stage to implement a structural core-to-periphery model: score = centrality 1–10 (NULL valid), inclusion = presence in working list. Score 0 deprecated. `auto_score_by_description` rewritten to call `reject_articles` instead of writing 0. `score_all_unscored` kept but docstring-deprecated. All 5 gather tools surface `rejected_sample: [{title, reason}, ...]` alongside `rejected_skipped` count via new `_apply_rejections` helper. Server instructions rewritten around the two-axis framing. IV handoff doc gets a centrality-roadmap section (enriched CSV today, future filter slider). Opens the path for IV to filter by threshold without requiring any IV change yet.
- **2026-04-22** — Stage 3: cooperative time budget on heavy tools (`[plan 3.1, 3.2, 3.3, 3.4]`). `get_category_articles` + `preview_category_pull` gain `time_budget_s=240` with transparent `exclude=<fully-crawled>` resume (3.1). `filter_articles` gains phase-based resume — on timeout each phase's partial work is discarded; `resume_suggestion.call_with` names completed phases as False (3.2). `harvest_list_page` + `preview_harvest_list_page` gain budget on the `prop=links` fallback path; `main_content_only=True` default remains single-call-bounded (3.3). `fetch_descriptions` marked ☑ retroactively — budget shipped in 1.6 (3.4).
