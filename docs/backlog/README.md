...
- **Expose exemplars as MCP `@mcp.resource()` instead of `list_exemplars` / `get_exemplar` tools.** Exemplar bodies are static markdown under `dogfood/exemplars/` — perfect resource shape (`exemplar://{slug}`). Resources let clients enumerate available reference material without an explicit tool call, and align with the skill's recommendation. Deferred because the existing tool surface works and migration means deprecation churn (keep both → worse; replace tools → breaks current dogfood call patterns). Promote when (a) we want richer client-side surfacing of "available exemplars" in a UI, or (b) we're already touching the exemplar tool surface for another reason. `[NEW — 2026-05-03, server-audit Tier 1.1.f]`
- **`wikidata_entities_by_property` soft limit measures compact-JSON, but response is returned indented.** `_WIKIDATA_RESPONSE_SOFT_LIMIT = 40_000` checks `_json_size()` (compact `separators=(',',':')`), but the actual return is `json.dumps(..., indent=2)`. Empirical ratio observed during 2026-05-03 eval-authoring: 426-row Cattleya P171 query had compact-trimmed = 35kb (under threshold, no further action) but indented wire = 61kb (overflowed client buffer). Two-line fix: either lower threshold to ~22kb (compact ≈ 40kb indented at the typical 1.7× ratio), or have `_json_size` measure the indented form. Surgical; one afternoon. `[NEW — 2026-05-03, server-audit Tier 1.2 refinement]`
- **`wikidata_get_entity` should optionally suppress sitelinks.** When `properties=[<short list>]`, the response still includes the full ~80-language `sitelinks` blob, dominating payload. Add `include_sitelinks: bool = True` parameter, or auto-suppress when `properties` filter is set and short. Two-agent evidence from 2026-05-03 evals (Q1 Greta Thunberg P569, Q3 IPCC P571 — both narrow date lookups). `[NEW — 2026-05-03, server-audit Tier 2.2]`
