...
- **Auth interaction.** Once TB has auth (see `docs/backlog/auth.md`), `publish_topic` should record the publishing user. IV could then assert the authenticated user matches, or leave it to IV admins. Separate the two concerns: TB auth lands first; IV import auth is its own policy.
